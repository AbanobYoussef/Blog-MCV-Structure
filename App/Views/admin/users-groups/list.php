...
<?php foreach ($users_groups as $US) { ?>
<div class='catHead' style=" margin-bottom:-35px">
      <ul class="row list-inline list" style="
        text-align: center;">
        <li style="width: 10%"><?php echo $US->id?></li>
        <li style="width: 30%"><?php echo $US->name ?></li>
       <!-- <li style="width: 25%"><?php echo $US->status ?></li>-->
         <li style="width: 20%; margin-left:20px;background-color: #3c8dbc;">
          <button class="btn btn-secondary open-popu"
                  style="width: 48%;display: inline-block;"
                  data-target="<?php echo url('/admin/users-groups/edit/'. $US->id);?>">
                  Edit
          </button>
          <?php if($US->id!=1) {?>
            <button class="btn btn-warning delete"         data-target="<?php echo url('/admin/users-groups/delete/'. $US->id);?>"
